...
export default function Home() {
  ...
  const payroll = filtered.reduce((sum, r) => { const p = people.find(x => x.id === r.personId); return sum + (p?.role === "正職" ? 0 : workMinutes(r,scheduleFor(r)) / 60 * (p?.rate || 0)); }, 0);
  ...
  function exportCsv() {
    ...
    exportPeople.forEach((person,index) => setTimeout(() => {
      ...
      const lines = [["日期","星期","姓名","身分","每日排班","打卡上班","打卡下班","出勤工時","計薪方式","預估薪資","狀態","備註"], ...personRows.map(r => { const s=scheduleFor(r); const wm=workMinutes(r,s); return [r.date,`星期${weekday(r.date)}`,person.name,person.role,s?`${s.start}-${s.end}`:"未排班",r.clockIn,r.clockOut,(wm/60).toFixed(2),person.role==="正職"?"固定月薪":`時薪 ${person.rate}`,person.role==="正職"?"固定月薪，不隨工時變動":Math.round(wm/60*person.rate),r.confirmed?"已核對":"待核對",r.note]; })];
  ...
  return <main>
    ...
    <section className="stats">
      ...
      <article><span>計時人員預估薪資</span><strong>{money(payroll)}</strong><small>正職採固定月薪，不隨工時變動</small></article>
    ...
    <section className="workspace">
      ...
      <div className="table-wrap"><table><thead><tr><th>日期</th><th>人員</th><th>身分</th><th>計薪排班</th><th>打卡時間</th><th>計薪工時</th><th>預估薪資</th><th>核對狀態</th><th>備註</th></tr></thead><tbody>
        {filtered.map(r=>{const p=people.find(x=>x.id===r.personId)!;const s=scheduleFor(r);const wm=workMinutes(r,s);return <tr key={r.id}><td><b>{r.date.slice(5).replace("-","/")}</b><small>星期{weekday(r.date)}</small></td><td><span className="avatar">{p.name[0]}</span><b>{p.name}</b></td><td><span className="tag">{p.role}</span></td><td>{s?<>{s.start}–{s.end}</>:<span className="warn">未設定</span>}</td><td><b>{r.clockIn}–{r.clockOut}</b>{s&&(minutes(r.clockIn)<minutes(s.start)||minutes(r.clockOut)>minutes(s.end))&&<small className="warn">超出排班不計薪</small>}</td><td><b>{fmtHours(wm)}</b></td><td>{p.role==="正職"?<><b>固定月薪</b><small>不隨工時變動</small></>:<><b>{money(wm/60*p.rate)}</b><small>時薪 {p.rate}</small></>}</td><td><button className={r.confirmed?"status ok":"status"} onClick={()=>setRows(rows.map(x=>x.id===r.id?{...x,confirmed:!x.confirmed}:x))}>{r.confirmed?"✓ 已核對":"! 待核對"}</button></td><td>{r.note||"—"}</td></tr>})}
        {!filtered.length&&<tr><td colSpan={9} className="empty">此期間沒有符合條件的紀錄</td></tr>}
      </tbody></table></div>
      <div className="table-footer"><span>共 {filtered.length} 筆紀錄 · 正職工時不影響固定月薪</span><b>總工時 {fmtHours(totalMin)}　計時薪資 {money(payroll)}</b></div>
    </section>

    <section className="people-section"><div className="section-title"><div><span className="eyebrow">TEAM & SCHEDULE</span><h3>人員與每日排班</h3></div><button className="text-btn" onClick={()=>setShowSchedule(true)}>設定每日排班 →</button></div><div className="people-grid">{people.map(p=>{const s=schedules.find(x=>x.personId===p.id&&x.date===iso(today));return <article key={p.id}><div className="person-head"><span className="avatar large">{p.name[0]}</span><div><b>{p.name}</b><small>{p.role} · {p.role==="正職"?"固定月薪":`時薪 ${money(p.rate)}`}</small></div><button onClick={()=>{setEditing(p);setShowSchedule(true)}}>•••</button></div><div className="schedule"><span>今日排班</span><strong>{s?`${s.start} — ${s.end}`:"今日未排班"}</strong></div></article>})}</div></section>
